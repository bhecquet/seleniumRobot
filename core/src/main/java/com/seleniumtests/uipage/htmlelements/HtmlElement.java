...
public class HtmlElement extends Element implements WebElement, Locatable {
	 ...
	 *              null, use <code>driver.findElement(By)</code> intenally If index
	 ...
	 *               null, use <code>driver.findElement(By)</code> intenally If
	 ...
	 *                      <code>driver.findElement(By)</code> intenally If index
	 *                      is negative, search from the last one (-1) If index is
	...
	public String findLink(String attributeName) {
		...
		// link <a href="#" id="linkPopup2"
		// onclick="window.open('http://www.infotel.com/', '_blank');">
		String link = findPattern(Pattern.compile(".*(http://.+?)'\"?.*"), attributeName);
		if (!"".equals(link)) {
			return link;
		}

		// link with simple quotes <a href="#" id="linkPopup"
		// onclick='window.open("http://www.infotel.com/", "_blank");'>
