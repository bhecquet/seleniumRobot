...
public abstract class CommonReporter implements IReporter {
    ...
    public static String getTestName(ITestResult testResult) {
        ...
        // <W>ith a testName we can return instead of N-A.
        try {
    ...
    public static String getVisualTestName(ITestResult testResult) {
        ...
        // [OUTILTEST-353] Dans le cas d'une erreur sur le serveur de variable, on peut arriver à ce point avec un test
        // qui ne soit pas skipped mais qui ait quand même un testName à retourner plutôt que N-A. Si possible, essayer
        // de retourner l'info la plus précise possible.
        try {
            if (testResult.getName() != null && !testResult.getName().isEmpty()) {
                return testResult.getName();
            } else {
