...
public abstract class EmailClientImpl implements EmailClient {
    ...
    @Override
    public List<String> checkMessagePresenceInLastMessagesByBody(String content, String[] attachmentNames, Email emailOut, int timeoutInSeconds) throws Exception {
        return checkMessagePresenceInLastMessagesByBody(content, Arrays.asList(attachmentNames), emailOut, timeoutInSeconds);
    }

    @Override
    public List<String> checkMessagePresenceInLastMessagesByBody(String content, String[] attachmentNames, Email emailOut) throws Exception {
        return checkMessagePresenceInLastMessagesByBody(content, Arrays.asList(attachmentNames), emailOut, 90);
    }
    ...
    @Override
    public List<String> checkMessagePresenceInLastMessagesByBody(String content, List<String> attachmentNames, Email emailOut, int timeoutInSeconds) throws Exception {
        List<Email> emailList = getEmailsByContent(content);
        Map<Email, List<String>> missingAttachmentsPerEmail = new HashMap<>();
        ...
        while (end.isAfter(clock.instant())) {
            ...
            // not found retry with last received emails
            emailList.addAll(getEmailsByContent(content));
            WaitHelper.waitForSeconds(10);
        }
    ...
    /**
     * Returns list of newly received email whose content contains param content
     *
     * @param content to find
     * @return email list
     * @throws Exception
     */
    public List<Email> getEmailsByContent(String content) throws Exception {
        List<Email> matchingEmails = new ArrayList<>();
        ...
        for (Email email : lastEmails) {
            String contentAll = email.getContent();

            // does content matches
            if (contentAll.contains(content)) {
                matchingEmails.add(email);
            }
        }
        return matchingEmails;
