...
public class TestDriverExtractor extends MockitoTest {
	...
	public void testDriverExtraction() throws IOException {
		...
		if (OSUtility.isWindows()) {
			Assert.assertTrue(Paths.get(driverPath.toString(), "chromedriver_2.46_chrome-71-73.exe").toFile().exists());
		} else {
			Assert.assertTrue(Paths.get(driverPath.toString(), "chromedriver_2.46_chrome-71-73").toFile().exists());
		}
		Assert.assertTrue(Paths.get(driverPath.toString(), "version_chromedriver_2.46_chrome-71-73.txt").toFile().exists());
	}
