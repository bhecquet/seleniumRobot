...
public class TestTestVariable extends GenericTest {
	...
	public void testFromJSonCustomVariable() {
		...
		Assert.assertEquals(variable.getName(), "key");
		Assert.assertEquals(variable.getValue(), "value");
		Assert.assertNull(variable.getRemoteFileUrl());
		Assert.assertFalse(variable.isReservable());
	...
	public void testFromJSonWithFile() {
		...
		Assert.assertEquals(variable.getId(), (Integer) 1);
		Assert.assertEquals(variable.getName(), "key");
		Assert.assertEquals(variable.getRemoteFileUrl(), "http://127.0.0.1:8000/media/appName/fileName.ext");
		Assert.assertFalse(variable.isReservable());
		Assert.assertEquals(variable.getInternalName(), "key");
	}
	...
	public void testVariableWithNullValue() {
		TestVariable tv = new TestVariable("nullvalue", (String) null);
		Assert.assertEquals(tv.getName(), "nullvalue");
		Assert.assertEquals(tv.getValue(), ""); // getValue returns empty string instead of null
	}

	@Test(groups = {"ut"})
	public void testSetValueAsFile() throws IOException {
		File tmpFile = Files.createTempFile("tmp", "tmp").toFile();
		tmpFile.deleteOnExit();
		TestVariable tv = new TestVariable("stringValue", "value");
		tv.setValue(tmpFile);
		Assert.assertEquals(tv.getValue(), tmpFile);
		Assert.assertEquals(tv._getFile(), tmpFile);
		Assert.assertEquals(tv._getValue(), "");
	}

	@Test(groups = {"ut"})
	public void testSetValueAsString() throws IOException {
		File tmpFile = Files.createTempFile("tmp", "tmp").toFile();
		tmpFile.deleteOnExit();
		TestVariable tv = new TestVariable("stringValue", tmpFile);
		tv.setValue("foo");
		Assert.assertEquals(tv.getValue(), "foo");
		Assert.assertNull(tv._getFile());
		Assert.assertEquals(tv._getValue(), "foo");
	}

	@Test(groups = {"ut"})
	public void testCopyVariable() throws IOException {
		File tmpFile = Files.createTempFile("tmp", "tmp").toFile();
		tmpFile.deleteOnExit();
		TestVariable tv = new TestVariable("stringValue", tmpFile);
		TestVariable copy = tv.copy("newStringValue");
		Assert.assertEquals(copy.getName(), "newStringValue");
		Assert.assertEquals(copy.getValue(), tmpFile);
		Assert.assertEquals(copy._getFile(), tmpFile);
		Assert.assertNull(copy._getValue());
	}

	@Test(groups = {"ut"})
	public void testCopyVariable2() {
		TestVariable tv = new TestVariable("stringValue", "bar");
		TestVariable copy = tv.copy("newStringValue");
		Assert.assertEquals(copy.getName(), "newStringValue");
		Assert.assertEquals(copy.getValue(), "bar");
		Assert.assertNull(copy._getFile());
		Assert.assertEquals(copy._getValue(), "bar");
	}
}