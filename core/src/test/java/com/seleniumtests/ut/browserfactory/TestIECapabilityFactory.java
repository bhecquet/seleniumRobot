...
public class TestIECapabilityFactory extends MockitoTest {
	...
	public void init() {
		...
		browserInfos.put(BrowserType.INTERNET_EXPLORER, List.of(new BrowserInfo(BrowserType.INTERNET_EXPLORER, "11", "", false)));
	...
	public void testCreateIECapabilitiesStandardDriverPathLocal() {
		try (MockedStatic<Advapi32Util> mockedAdvapi = mockStatic(Advapi32Util.class)) {
			mockedAdvapi.when(() -> Advapi32Util.registryGetValue(any(HKEY.class), anyString(), anyString())).thenReturn("1");
	...
	public void testCreateDefaultEdgeIEModeCapabilities() {
		...
		Map<BrowserType, List<BrowserInfo>> browserInfos = new EnumMap<>(BrowserType.class);
		browserInfos.put(BrowserType.INTERNET_EXPLORER, List.of(new BrowserInfo(BrowserType.INTERNET_EXPLORER, "11", "", false)));
		browserInfos.put(BrowserType.EDGE, List.of(new BrowserInfo(BrowserType.EDGE, "97.0", "", false)));
		mockedOsUtility.when(() -> OSUtility.getInstalledBrowsersWithVersion(false)).thenReturn(browserInfos);
