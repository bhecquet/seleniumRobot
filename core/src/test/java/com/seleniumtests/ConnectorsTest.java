...
public class ConnectorsTest extends MockitoTest {
	...
	private HttpRequest<?> preparePostRequest(String serverUrl, String responseType, HttpRequestWithBody postRequest, HttpResponse<String> response, HttpResponse<JsonNode> jsonResponse) {
		...
		when(postRequest.body(anyString())).thenReturn(requestBodyEntity);
	...
	protected void configureMockedVariableServerConnection(String serverUrl) throws UnirestException {
		...
		variablesRequest = (GetRequest) createServerMock(serverUrl, "GET", SeleniumRobotVariableServerConnector.VARIABLE_API_URL, 200, "[{'id': 1, 'name': 'key1', 'value': 'value1', 'uploadFile': null, 'reservable': false}, {'id': 2, 'name': 'key2', 'value': 'value2', 'reservable': true}, {'id': 3, 'name': 'key3', 'value': '', 'uploadFile': 'http://127.0.0.1:8000/media/appName/testStandardDataProvider.csv', 'reservable': false}, {'id': 4, 'name': 'key4', 'value': '', 'uploadFile': 'http://127.0.0.1:8000/media/appName/null.csv', 'reservable': false}]");		
		...
		variableFileRequest = (GetRequest) createServerMock(serverUrl, "GET", String.format(SeleniumRobotVariableServerConnector.VARIABLE_FILE_API_URL, 4), 404, "");
