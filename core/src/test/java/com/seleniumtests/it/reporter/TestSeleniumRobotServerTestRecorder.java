...
public class TestSeleniumRobotServerTestRecorder extends ReporterTest {
	...
	public void testReportGeneration() throws Exception {
		...
		try (MockedConstruction<SeleniumRobotVariableServerConnector> mockedVariableServer = mockConstruction(SeleniumRobotVariableServerConnector.class, (variableServer, context) -> {
			...
			verify(serverConnector, times(35)).recordStepResult(any(TestStep.class), anyInt(), anyInt()); // all steps are recorded
			...
			verify(serverConnector).updateStepResult(contains("\"snapshots\":[{\"durationToExclude\":0,\"idHtml\":0,\"displayInReport\":true,\"name\":\"a name\",\"idImage\":0,\"failed\":false,\"position\":0,\"type\":\"snapshot\",\"snapshotCheckType\":\"NONE\""), eq(0));
			verify(serverConnector).updateStepResult(contains("\"snapshots\":[{\"durationToExclude\":0,\"idHtml\":null,\"displayInReport\":true,\"name\":\"main\""), eq(0));
	...
	public void testReportContainsDownloadedFile() throws Exception {
		...
		try (MockedConstruction<SeleniumRobotVariableServerConnector> mockedVariableServer = mockConstruction(SeleniumRobotVariableServerConnector.class, (variableServer, context) -> {
			...
			Assert.assertTrue(fileCapture.getAllValues().stream().anyMatch(f -> f.getName().equals("nom-du-fichier.pdf")));
	...
	private void initMocks(MockedStatic<CommonReporter> mockedCommonReporter, MockedStatic<SeleniumRobotSnapshotServerConnector> mockedServerConnector) throws Exception {
