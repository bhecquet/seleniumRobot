...
| allowConcurrentTestsOnGrid   | false      | If true, allow to bypass the max-session parameter on grid node, up to the limit fixed to Selenium. It is only useful when max-sessions=1. In this case, only tests that declare the this parameter will be allowed to run on the same node<br/>**This will only apply to tests where `testRetryCount` > 0 and for the first execution, so that, on retry, video capture can be taken on failing test**                                                      |                                                                                                                                                                                                                                 |   
...
| userAgent 				| 			| Allow defining a specific user-agent in chrome and firefox only. It can contain any variable available in the SeleniumTestsContext written as ${foo}, it will be automatically interpolated. For example, the parameter -DuserAgent="SeleniumRobot ${browser} - ${testName}" will result as "SeleniumRobot Chrome - myPrettyTest".    | 
...
- `disable-background-networking`: this option prevent chrome/edge to download managed extensions (Set by default by Selenium)
- `disable-web-security`: may fail OAuth authentication on some web sites (Set by default by SeleniumRobot)
...
**relaxed-security allow access to "/appium/sessions" endpoint and driver autodownload. But if you are in an insecure environment, provide a finer flag: https://appium.io/docs/en/3.0/guides/security/**
