...
//	}
    ...
    /**
     * Check for issue #47 where ReplayAction aspect raised an error when switching
     * to default context after click with alert present
     */
    ...
    public void testFindWithAlert() {
        ...
        if (((CustomEventFiringWebDriver) driver).getWebDriver() instanceof FirefoxDriver
                && FirefoxDriverFactory.isMarionetteMode()) {
    ...
    /**
     * Check that if no element is returned, no error is raised but we should have
    ...
    /**
     * Tests NOT finding sub-elements of an HTMLElement No exception should be
     * raised but search should be done several times
     */
    ...
    /**
     * Tests finding sub-elements of an HTMLElement inside frame issue #314: search
     * for an element into iframe. Search sub-elements of this element. We search in
     * the second table because the bug relies in the fact that findHtmlElements do
     * return elements which do not correspond the the elements returned by a simple
     * driver.findElements(By.tagname('a'))
     */

    public void testFindHtmlElementsByInsideFrame() {
        List<WebElement> htmlElements = new HtmlElement("", By.id("tableIframe2"), DriverTestPage.iframe)
                .findHtmlElements(By.tagName("td"));
        Assert.assertEquals(htmlElements.size(), 2);
        Assert.assertTrue(htmlElements.get(0) instanceof HtmlElement);
    ...
    public void testFindPattern4() {
        Assert.assertEquals(
                new HtmlElement("", By.id("divFind2")).findPattern(Pattern.compile("an (\\w+) text"), "text"), "other");
    }
    ...
    /**
     * Test that it's possible to use an hidden element. Make it appear before using
     * it
     */
    ...
    public void testWebDriverWaitWithLowTimeout() {
        ...
        // we cannot check precise timing as it depends on the hardware, but we should
        // never wait more that 10 secs (the default timeout for searching element is 30
        // secs)
    ...
    public void testElementNotPresent() {
        ...
        try {
            ...
            Assert.assertTrue(e.getMessage().contains(
                    "Searched element [HtmlElement element, by={By.id: notPresent}] from page 'com.seleniumtests.it.driver.support.pages.DriverTestPage' could not be found"));
            throw e;
        } finally {
...
//	}
    ...
    /**
     * Vérifie qu'avant d'agir sur un élément, on positionne la fenêtre du
     * navigateur pour qu'il soit visible
     */
    ...
    /**
     * Test file upload in case selenium method does not work. This uses the java
     * Robot version which controls mouse and keyboard
     *
     * @throws AWTException
...
//		DriverTestPage.upload.click();
        DriverTestPage.uploadedFile.click(); // when executing both testUploadFileWithRobotXX tests, the second one
        // fails on firefox because focus is on '<input type="file">' element
        DriverTestPage.upload.clickAction(); // due to restrictions clicking a <input type="file"> element with firefox,
    ...
    /**
     * Force driver to use full keyboard typing when writing text instead of using
     * copy-paste feature
     *
     * @throws AWTException
     * @throws InterruptedException
...
//		DriverTestPage.upload.click();
    ...
    /**
     * Check that if FIRST_VISIBLE is specified, first visible element is returned.
     * In this case, FIRST_VISIBLE has only been applied to the parent element
     * refresh page to be sure element has not been made visible
     */
    ...
    /**
     * issue #166: Check that when searching an element by XPath, and this element
     * is specified as being located in an other element, we still get the element
     * with an xpath relative to the parent element
     */
    ...
    /**
     * Check that when searching an element by XPath, and this element is specified
     * as being located in an other element, we still get the element with an xpath
     * relative to the parent element. In this case, the xpath is already specified
     * as relative. No modification should be done by robot
     */
    ...
    /**
     * Check search by XPath without parent is correctly performed (no change due to
     * correction of issue #166)
     */
    ...
    /**
     * issue #355: check that we get "false" when element is not present and index
     * is given
     */
    ...
    /**
     * issue #274: check that scrolling is correct and we can click on an element at
